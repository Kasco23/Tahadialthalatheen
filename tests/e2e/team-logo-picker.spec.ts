...
test.describe("TeamLogoPicker Integration", () => {
  ...
  test("should display Real Madrid logo from La Liga folder", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("text=No team logos available"),
    ).not.toBeVisible();
  ...
  test("should complete full flow: pick logo, join as player, show in lobby", async ({
    page,
  }) => {
