  ...
  useEffect(() => {
    ...
    const fetchStrikes = async () => {
      try {
        ...
        const strikesMap = data.reduce((acc, strike) => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            setParticipants(prev => 
              prev.map(participant => 
                participant.participant_id === newData.participant_id ? newData : participant
              )
            )
          } else if (payload.eventType === 'DELETE') {
            const oldData = payload.old as Tables<'Participant'>
            setParticipants(prev => 
              prev.filter(participant => participant.participant_id !== oldData.participant_id)
            )
          }
        }
      )
