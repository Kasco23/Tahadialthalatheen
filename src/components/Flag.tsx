...
export const Flag: React.FC<{
  ...
  if (!loaded) {
    ...
    return (
      <span className={`inline-block w-6 h-4 bg-gray-300 rounded ${className}`} title="Loading flag..."></span>
    );
  }

  if (!code) {
    // Return a default flag if no code provided
    return <span className={`fi fi-sa ${className}`} title="Default flag"></span>;
  }
  ...
  return <span className={`fi fi-${flagCode} ${className}`} title={`Flag: ${normalizedCode}`}></span>;
};
