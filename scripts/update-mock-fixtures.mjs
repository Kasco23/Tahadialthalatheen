...
} = await import("../src/lib/api/transfermarktCache.ts");
...
try {
  ...
  console.log(
    `   Found ${cityPlayers.length} Man City players:`,
    cityPlayers.slice(0, 5),
  );
  ...
  const goalkeeper = madridSquad.players.find(
    (p) => p.position === "Goalkeeper" || p.position === "Keeper",
  );
  ...
  const teamSquadDataFallback = [
    {
      question: "Name 5 players from Manchester City's 2024-25 squad",
      answer:
        "Multiple correct answers (e.g., Haaland, De Bruyne, Rodri, Foden, Walker)",
      difficulty: "medium",
      category: "Squads",
      metadata: {
        team: "Manchester City",
    ...
    },
    {
      question: "List 3 defenders from Liverpool's current squad",
      answer:
        "Multiple correct answers (e.g., Van Dijk, Konaté, Alexander-Arnold)",
      difficulty: "medium",
      category: "Squads",
      metadata: {
        team: "Liverpool",
